...
Current phase: `PHASE_1_COLLECTION`  
Workflow: `continuous-worker-v2`  
Full archive collection: **RESTARTED AS ORIGINAL-LIVESTREAM DATA COLLECTION ONLY**

## Sealed historical material

Previous Pilot/alignment/playback files are sealed historical material. They remain in the
repository for audit history, but they must not be used as current progress, current task input,
or active database input.

Current active data must be written under:

```text
data/current/
```

The SQLite builder reads only `data/current/`. Files outside `data/current/` are not current
...
Do not infer state from stale chats, sealed Pilot files, or historical completion files alone.
The current repository control plane and `data/current/` are authoritative for this restarted run.
...
After finishing any one task, the worker must commit/push or PR-publish the durable output,
refresh `main` plus open PR state, run task discovery again, and claim/reserve the next eligible
task. Completing one task, one livestream, one batch, one Playback request, one text-crosscheck
pass, or one PR update is not a valid final stop by itself.

If the next candidate is already claimed or PR-reserved, classify the race, refresh, and select
another eligible task without asking the user whether to continue.
