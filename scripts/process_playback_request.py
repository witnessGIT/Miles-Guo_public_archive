...
def collect_frame_evidence(clip: Path, temp_dir: Path, decode_start: float, expected: float, window: float) -> list[dict]:
    results: list[dict] = []
...
def build_markdown(evidence: dict) -> str:
    ...
    lines = [
        ...
        f"- Expected start: `{evidence['expected_start_sec']:.3f}s`",
        f"- Decode window: `{evidence['decode_start_sec']:.3f}s` → `{evidence['decode_end_sec']:.3f}s`",
        ...
        "## Target text",
        "",
        evidence.get("target_text") or "(none)",
        "",
    ...
    if match:
        lines.extend([
            f"- Candidate observed position: `{evidence['candidate_observed_position_sec']:.3f}s`",
            f"- Signed timing error: `{evidence['candidate_timing_error_sec']:+.3f}s`",
...
def process_request(path: Path, *, force: bool = False) -> Path:
    ...
    if decode_proc.returncode != 0 or not decode.get("ok"):
        failed = {
            ...
            "status": "blocked_media_decode",
            **ids,
            "request_ref": safe_rel(path),
            "media_url": media_url,
        ...
        (out_dir / "evidence.md").write_text(build_markdown({
            **failed,
            ...
            "decode_start_sec": expected,
            "decode_end_sec": expected,
    ...
    if audio_proc.returncode == 0 and shutil.which(whisper_cli) and whisper_model:
        whisper_proc = run([
            whisper_cli,
            "-m", whisper_model,
            "-f", str(audio),
            "-l", str(request.get("language", "zh")),
            "-osrt",
            "-of", str(whisper_prefix),
            "--no-prints",
        ])
    entries = parse_srt(srt_path)
    target_text = str(
        segment.get("text_curated")
        or segment.get("text_asr")
        or segment.get("text_search")
        or ""
    )
    ...
    timing_error = None
    if match:
        observed = round(decode_start + float(match["local_start_sec"]), 3)
        timing_error = round(observed - expected, 3)
    ...
    for entry in entries:
        audio_rows.append({
            "local_start_sec": entry["start_sec"],
            "local_end_sec": entry["end_sec"],
            "absolute_start_sec": round(decode_start + entry["start_sec"], 3),
            "absolute_end_sec": round(decode_start + entry["end_sec"], 3),
            "text": entry["text"],
        })
    ...
    core = {
        ...
        **ids,
        ...
        "target_text_search": segment.get("text_search"),
        "expected_start_sec": expected,
        ...
        "repository_content_inspection": True,
        "inspection_engine": "github-actions+ffmpeg+whisper.cpp+frame-ocr",
        "audio_asr": {
            "engine": "whisper.cpp",
            "model": os.path.basename(whisper_model) if whisper_model else None,
            "entries": audio_rows,
            "best_match": match,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Turn repository playback requests into durable Agent-readable decoded-media evidence."
    )
